...
const regex = /\/\/ Process referral first if applicable\s+if \(text\.startsWith\("\/start"\)\) \{[\s\S]*?if \(!\(await ensureMembership\(baseUrl, chatId\)\)\) return;/;
...
if (match) {
  ...
  console.log("Patched start logic dynamically");
} else {
  console.log("Could not find start logic block with regex");
}
